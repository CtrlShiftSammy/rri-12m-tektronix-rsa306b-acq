...
def parse_20cm_sources(filename):
    ...
    while i < len(lines):
        ...
        if re.match(r'^\d{4}[+-]\d{3}.*J2000', line):
            ...
            ra = parts[3]
            dec = parts[4]
            # Look ahead for 20cm band
            j = i + 1
            flux_20cm = None
            while j < len(lines) and not re.match(r'^\d{4}[+-]\d{3}.*J2000', lines[j]):
                band_line = lines[j]
                if band_line.strip().startswith('20cm'):
                    # Try to extract FLUX(Jy) value (last number in line)
                    m = re.search(r'20cm\s+\S+\s+([^\d]*)([\d\.]+)', band_line)
                    if m:
                        flux_20cm = float(m.group(2))
                    else:
                        # fallback: last number in line
                        nums = re.findall(r'[\d\.]+', band_line)
                        if nums:
                            flux_20cm = float(nums[-1])
                    break
                j += 1
            if flux_20cm is not None:
                sources.append((name, ra, dec, flux_20cm))
        i += 1
    return sources
...
sources = parse_20cm_sources('VLA_calibrator_list.txt')
...
plt.colorbar(sc, label='Flux (Jy) at 20cm')
plt.xlim(0, 360)
plt.ylim(-90, 90)
...
plt.title('VLA Calibrator Sources with 20cm Band (ICRS Coordinates)')
...
with open("crab_dec_window_20cm_sources.txt", "w") as f:
...
with open("vela_dec_window_20cm_sources.txt", "w") as f:
    f.write("Source_Name\tRA_(deg)\tDec_(deg)\tFlux_(Jy)\n")
...
plt.colorbar(sc2, ax=ax, orientation='horizontal', pad=0.08, label='Flux (Jy) at 20cm')
...
ax.set_title('VLA Calibrator Sources with 20cm Band (Galactic Coordinates)')
ax.grid(True)
plt.tight_layout()
ax.legend()
plt.show()